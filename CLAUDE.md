...
- `src/commands/workflow.rs` — `pm workflow show` (prints active workflow's `workflow.md`, then appends a `## summary.md` brevity note from the single-source `SUMMARY_GUIDANCE` const so the summary owner sees it through the command they actually run) and `pm workflow list` (lists installed workflows with descriptions). Used by the bundled `pm-workflow` skill so agents can discover their per-feature routing at the start of every task.
